...
const SafetyIndex: React.FC = () => {
  ...
  const previousIndex = safetyData.length > 1 ? 
    parseInt(safetyData[safetyData.length - 2]?.safety_index || '0') : 0;
  const improvement = ((safetyIndex - previousIndex) / Math.max(previousIndex, 1) * 100).toFixed(1);
