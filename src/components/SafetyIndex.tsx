...
import safetyData from '@/data/safety_index.csv';

const SafetyIndex: React.FC = () => {
  // Get the latest safety index entry
  const latestEntry = safetyData[safetyData.length - 1];
  const safetyIndex = latestEntry?.safety_index || 0;
  
  // Calculate improvement
  const previousIndex = safetyData.length > 1 ? safetyData[safetyData.length - 2]?.safety_index : 0;
  const improvement = ((safetyIndex - previousIndex) / previousIndex * 100).toFixed(1);
